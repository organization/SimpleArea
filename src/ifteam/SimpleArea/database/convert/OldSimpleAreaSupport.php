...
class OldSimpleAreaSupport {
	...
	public function convertYMLtoJSON() {
		...
		foreach ($worlds as $worldName) {
			$link = $this->server->getDataPath() . "worlds/{$worldName}/options.yml";
			$jsonLink = $this->server->getDataPath() . "worlds/{$worldName}/options.json";
			...
			$link = $this->server->getDataPath() . "worlds/{$worldName}/protects.yml";
			$jsonLink = $this->server->getDataPath() . "worlds/{$worldName}/protects.json";
			if (file_exists($link) and !file_exists($jsonLink)) {
