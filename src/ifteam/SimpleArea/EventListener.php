...
class EventListener implements Listener {
    ...
    public function onMove(PlayerMoveEvent $event) {
        ...
        } else if (abs(($posX) - ($this->queue['movePos'][$user]['x'])) > 2 || abs(($posZ) - ($this->queue['movePos'][$user]['z'])) > 2) {
        ...
        $msg = '';
        $area = $this->areaProvider->getArea($world, $posX, $posZ, $user);
        ...
        // Rent Part
        $rent = $this->rentProvider->getRent($world, $posX, $posY, $posZ);
    ...
    public function onCommand(CommandSender $player, Command $command, string $label, Array $args): bool {
        ...
        switch (strtolower($command->getName())) {
            case $this->get("commands-area"):
                ...
                switch (strtolower($args[0])) {
                    ...
                    case $this->get("commands-area-manualcreate"):
                        ...
                        if (!isset($this->queue["manual"][strtolower($player->getName())])) {
                            $this->queue["manual"][strtolower($player->getName())] = [
                                    ...
                                    "startZ" => null,
                                    "endZ" => null,
                                    "startworld" => $player->getWorld()->getFolderName()
                            ];
                    ...
                    case $this->get("commands-area-sizeup"):
                        ...
                        $this->areaManager->areaSizeUp($player, $world, $id, $startX, $endX, $startZ, $endZ, $price);
                        if (isset($this->queue["areaSizeUp"][strtolower($player->getName())]))
                            unset($this->queue["areaSizeUp"][strtolower($player->getName())]);
                        break;
                    case $this->get("commands-area-sizedown"):
                        ...
                        $startZ = $sizeUpData["startZ"];
                        $endZ = $sizeUpData["endZ"];
                        $this->areaManager->areaSizeDown($player, $world, $id, $startX, $endX, $startZ, $endZ);
                        unset($this->queue["areaSizeDown"][strtolower($player->getName())]);
                        break;
                ...
                switch (strtolower($args[0])) {
                    ...
                    case $this->get("commands-whiteworld-info"):
                        if (!$player->hasPermission("simplearea.whiteworld.info"))
                            return false;
                        $this->whiteworldManager->info($player->getWorld(), $player);
                        break;

                    case $this->get("commands-whiteworld-protect"):
                        if (!$player->hasPermission("simplearea.whiteworld.protect"))
                            return false;
                        $this->whiteworldManager->protect($player->getWorld(), $player);
                        break;
                    ...
                    case $this->get("commands-whiteworld-areaprice"):
                        ...
                        $this->whiteworldManager->areaPrice($player->getWorld(), $args[1], $player);
                        break;
                    ...
                    case $this->get("commands-whiteworld-areaholdlimit"):
                        ...
                        $this->whiteworldManager->areaHoldLimit($player->getWorld(), $args[1], $player);
                        break;
                    ...
                    case $this->get("commands-whiteworld-defaultareasize"):
                        ...
                        $this->whiteworldManager->defaultAreaSize($player->getWorld(), $args[1], $args[2], $player);
                        break;
                    case $this->get("commands-whiteworld-accessdeny"):
                        if (!$player->hasPermission("simplearea.whiteworld.defaultareasize"))
                            return false;
                        $this->whiteworldManager->setAccessDeny($player->getWorld(), $player);
                        break;

                    case $this->get("commands-whiteworld-sizeup"):
                        if (!$player->hasPermission("simplearea.whiteworld.defaultareasize"))
                            return false;
                        $this->whiteworldManager->setAreaSizeUp($player->getWorld(), $player);
                        break;

                    case $this->get("commands-whiteworld-sizedown"):
                        if (!$player->hasPermission("simplearea.whiteworld.defaultareasize"))
                            return false;
                        $this->whiteworldManager->setAreaSizeDown($player->getWorld(), $player);
                        break;

                    case $this->get("commands-whiteworld-pvpallow"):
                        if (!$player->hasPermission("simplearea.whiteworld.pvpallow"))
                            return false;
                        $this->whiteworldManager->pvpAllow($player->getWorld(), $player);
                        break;
                    ...
                    case $this->get("commands-whiteworld-priceperblock"):
                        ...
                        $this->whiteworldManager->setPricePerBlock($player->getWorld(), $player, $args[1]);
                        break;

                    case $this->get("commands-whiteworld-checkshare"):
                        if (!$player->hasPermission("simplearea.whiteworld.checkshare"))
                            return false;
                        $this->whiteworldManager->setCountShareArea($player->getWorld(), $player);
                        break;
            ...
            case $this->get("commands-areatax"):
                ...
                $this->whiteWorldProvider->get($player->getWorld())
                        ->setAreaTax($args[0]);
                ...
                switch ($args[0]) {
                    ...
                    case $this->get("commands-rent-create"):
                        ...
                        if (isset($this->queue["rentCreate"][strtolower($player->getName())])) {
                            ...
                            $startworld = $this->queue["rentCreate"][strtolower($player->getName())]["startworld"];
                            $this->rentManager->create($player, $startworld, $startX, $endX, $startY, $endY, $startZ, $endZ, $areaId, $rentPrice);
                            if (isset($this->queue["rentCreate"][strtolower($player->getName())]))
                                unset($this->queue["rentCreate"][strtolower($player->getName())]);
                            return true;
                        }
    ...
    public function onBlockChangeEvent(Event $event) {
        ...
        if ($area instanceof AreaSection) {
            ...
            if ($area->isProtected()) {
                ...
                if (isset($type)) {
                    $ev = new AreaModifyEvent($event->getPlayer(), $event->getBlock()->getPos()
                            ->getWorld()
                            ->getFolderName(), $area->getId(), $event->getBlock(), $type);
                    $ev->call();
        ...
        if ($whiteWorld->isProtected()) {
            ...
            if (isset($type)) {
                $ev = new AreaModifyEvent($event->getPlayer(), $event->getBlock()->getPos()
                        ->getWorld()
                        ->getFolderName(), null, $event->getBlock(), $type);
                $ev->call();
                // $this->plugin->getServer()
                // ->getPluginManager()
                // ->callEvent($ev);
    ...
    public function onBlockBreakEvent(BlockBreakEvent $event) {
        $area = $this->areaProvider->getArea($event->getBlock()->getPos()->world, $event->getBlock()->getPos()->x, $event->getBlock()->getPos()->z, strtolower($event->getPlayer()
                ->getName()));
        if ($area instanceof AreaSection) {
            $rent = $this->rentProvider->getRent($event->getBlock()->getPos()->world, $event->getBlock()->getPos()->x, $event->getBlock()->getPos()->y, $event->getBlock()->getPos()->z);
            if (($rent instanceof RentSection) and !$rent->isBuySingNull()) {
                ...
                $blockPosString = "{$blockPos->getPos()->x}:{$blockPos->getPos()->y}:{$blockPos->getPos()->z}";
        ...
        switch (true) {
            case ($event->getBlock()->getID() == BlockLegacyIds::SIGN_POST):
                $sign = $event->getPlayer()
                        ->getWorld()
                        ->getTile($event->getBlock()->getPos());
                break;
            case ($event->getBlock()
                            ->getSide(Facing::UP)
                            ->getId() == BlockLegacyIds::SIGN_POST):
                $sign = $event->getPlayer()
                        ->getWorld()
                        ->getTile($event->getBlock()->getPos()
                                ->getSide(Facing::UP));
        ...
        if (isset($sign)) {
            ...
            if ($lines->getLine(0) == $this->get("automatic-post-1") && $lines->getLine(3) == $this->get("automatic-post-4"))
                if ($lines[2] == $this->get("automatic-post-3")) {
                    ...
                    if ($timeout <= 10) {
                        $size = explode("x", strtolower($lines->getLine(1)));
                        $dmg = $event->getBlock()->getMeta();
                        if ($dmg != 0 and $dmg != 4 and $dmg != 8 and $dmg != 12) {
                            if ($dmg < 4) {
                                $event->getBlock()->getPos()
                                        ->getWorld()
                                        ->setBlock($event->getBlock()->getPos(), BlockFactory::get(BlockLegacyIds::SIGN_POST, 0));
                            } else if ($dmg < 8) {
                                $event->getBlock()->getPos()
                                        ->getWorld()
                                        ->setBlock($event->getBlock()->getPos(), BlockFactory::get(BlockLegacyIds::SIGN_POST, 4));
                            } else if ($dmg < 12) {
                                $event->getBlock()->getPos()
                                        ->getWorld()
                                        ->setBlock($event->getBlock()->getPos(), BlockFactory::get(BlockLegacyIds::SIGN_POST, 8));
                            } else {
                                $event->getBlock()->getPos()
                                        ->getWorld()
                                        ->setBlock($event->getBlock()->getPos(), BlockFactory::get(BlockLegacyIds::SIGN_POST, 12));
                            }
                        }
                        $this->areaManager->destructPrivateArea($event->getPlayer(), $event->getBlock(), $size[0], $size[1], $event->getBlock()->getPos()->getY() - 1, $dmg);
    ...
    /* $text = new SignText([
             TextFormat::LIGHT_PURPLE . $this->get("rent-buy-sign-1"),
             TextFormat::LIGHT_PURPLE . $this->get("rent-buy-sign-2"),
             TextFormat::LIGHT_PURPLE . $this->get("rent-buy-sign-3"),
             TextFormat::LIGHT_PURPLE . $this->get("rent-buy-sign-4")
     ]);
     $event->setNewText($text);

     $rent->setBuySignPos($event->getBlock()->getPos()->x, $event->getBlock()->getPos()->y, $event->getBlock()->getPos()->z);
     $this->message($event->getPlayer(), $this->get("rent-buy-sign-set-complete"));
     return;
 }
}
$text = $event->getSign()->getText();
if ($text->getLine(0) == $this->get("automatic-post-1"))
 if ($text->getLine(2) == $this->get("automatic-post-3"))
     if ($text->getLine(3) == $this->get("automatic-post-4"))
         if (!$event->getPlayer()->isOp()) {
             $this->alert($event->getPlayer(), $this->get("automatic-post-is-only-create-op"));
             $event->setCancelled();
             return;
         }
if ($text->getLine(0) == $this->get("easy-automatic-post")) {
 if (!$event->getPlayer()->isOp()) {
     $this->alert($event->getPlayer(), $this->get("automatic-post-is-only-create-op"));
     $event->setCancelled();
     return;
 }
 if ($event->getBlock()->getId() != BlockLegacyIds::SIGN_POST) {
     $this->alert($event->getPlayer(), $this->get("wall-sign-doesnt-use-it"));
     $event->setCancelled();
     return;
 }
 $size = strtolower($text->getLine(1));
 $size = explode("x", $size);
  ...
  *//*
            $text = new SignText([
                    $this->get("automatic-post-1"),
                    $size[0] . $this->get("automatic-post-2") . $size[1],
                    $this->get("automatic-post-3"),
                    $this->get("automatic-post-4")
            ]);
            $event->setNewText($text);
            return;
        }
    ...
    public function onPlayerInteractEvent(PlayerInteractEvent $event) {
        if ($event->getAction() == PlayerInteractEvent::RIGHT_CLICK_BLOCK) {
            ...
            if (isset($this->queue['manual'][$lowerpname])) {
                $event->setCancelled();
                $startworld = $this->queue['manual'][$lowerpname]['startworld'];
                if ($startworld !== $player->getWorld()->getFolderName()) {
                ...
                if ($startX === null) {
                    $this->queue['manual'][$lowerpname]['startX'] = $event->getBlock()->getPos()->getX();
                    $this->queue["manual"][strtolower($event->getPlayer()->getName())]["startZ"] = $event->getBlock()->getPos()->getZ();
                    $this->message($player, $this->get("first-pos-choosed"));
                    return;
                }
                $endX = $this->queue['manual'][$lowerpname]['endX'];
                if ($endX === null) {
                    $this->queue['manual'][$lowerpname]['endX'] = $event->getBlock()->getPos()->getX();
                    $this->queue['manual'][$lowerpname]['endZ'] = $event->getBlock()->getPos()->getZ();

                    // $pricePerBlock = $this->whiteWorldProvider->get($player->getWorld())
                    // ->getPricePerBlock();
                ...
                $event->setCancelled();
                $startworld = $this->queue['rentCreate'][$lowerpname]['startworld'];
                if ($startworld !== $player->getWorld()->getFolderName()) {
                ...
                $area = $this->areaProvider->getArea($player->getWorld()
                        ->getDisplayName(), $touched->getPos()->x, $touched->getPos()->z, strtolower($event->getPlayer()
                        ->getName()));
                if (!$area instanceof AreaSection) {
                ...
                if ($startX === null) {
                    $this->queue['rentCreate'][$lowerpname]['startX'] = $event->getBlock()->getPos()->getX();
                    $this->queue['rentCreate'][$lowerpname]['startY'] = $event->getBlock()->getPos()->getY();
                    $this->queue['rentCreate'][$lowerpname]['startZ'] = $event->getBlock()->getPos()->getZ();
                    $this->message($player, $this->get('first-pos-choosed'));
                    $this->message($player, $this->get('you-can-stop-create-manual-area'));
                    return;
                }
                ...
                if ($endX === null) {
                    $this->queue['rentCreate'][$lowerpname]["endX"] = $event->getBlock()->getPos()->getX();
                    $this->queue['rentCreate'][$lowerpname]['endY'] = $event->getBlock()->getPos()->getY();
                    $this->queue['rentCreate'][$lowerpname]['endZ'] = $event->getBlock()->getPos()->getZ();
                ...
                if (!$sizeUpData['isTouched']) {
                    $area = $this->areaProvider->getAreaToId($sizeUpData['startworld'], $sizeUpData['id']);
                    ...
                    $touchX = $event->getBlock()->getPos()->x;
                    $touchZ = $event->getBlock()->getPos()->z;
                    ...
                    $xSize = $endX - $startX;
                    $zSize = $endZ - $startZ;
                    $whiteWorld = $this->whiteWorldProvider->get($sizeUpData['startworld']);
                ...
                if (!$sizeDownData['isTouched']) {
                    $area = $this->areaProvider->getAreaToId($sizeDownData['startworld'], $sizeDownData["id"]);
            ...
            $posZ = $block->getPos()->z;
            $rent = $this->rentProvider->getRent($world, $posX, $posY, $posZ);
            $area = $this->areaProvider->getArea($world, $posX, $posZ, strtolower($event->getPlayer()
                    ->getName()));
            ...
            if ($block->getID() == BlockLegacyIds::SIGN_POST) {
                $sign = $event->getPlayer()
                        ->getWorld()
                        ->getTile($event->getBlock()->getPos());
                if (!$sign instanceof Sign)
                ...
                switch ($sign->getBlock()->getMeta()) {
                    case 0: // 63:0 x+ z- XxZ
                        $startX = ($event->getBlock()->getPos()->x + 1);
                        $startZ = ($event->getBlock()->getPos()->z - 1);
                        $endX = ($event->getBlock()->getPos()->x + $xSize - 1);
                        $endZ = ($event->getBlock()->getPos()->z - $zSize + 1);
                        break;
                    case 4: // 63:4 x+ z+ ZxX
                        $startX = ($event->getBlock()->getPos()->x + 1);
                        $startZ = ($event->getBlock()->getPos()->z + 1);
                        $endX = ($event->getBlock()->getPos()->x + $zSize - 1);
                        $endZ = ($event->getBlock()->getPos()->z + $xSize - 1);
                        break;
                    case 8: // 63:8 x- z+ XxZ
                        $startX = ($event->getBlock()->getPos()->x - 1);
                        $startZ = ($event->getBlock()->getPos()->z + 1);
                ...
                $area = $this->areaProvider->addArea($world, $startX, $endX, $startZ, $endZ, $owner, $isHome, false);
                ...
                if ($area instanceof AreaSection) {
                    ...
                    $overlap = $this->areaProvider->checkOverlap($world, $startX, $endX, $startZ, $endZ);
            ...
            if ($event->getBlock()->getId() == BlockLegacyIds::SIGN_POST)
                return;
            if ($event->getBlock()->getId() == BlockLegacyIds::WALL_SIGN)
                return;
            if ($event->getBlock()->getId() == BlockLegacyIds::CRAFTING_TABLE)
                return;
            if ($event->getBlock()->getId() == BlockLegacyIds::FURNACE)
                return;
            $this->onBlockChangeEvent($event);
        }
    }
    ...
    public function onEntityDamageEvent(EntityDamageEvent $event) {
        ...
        $area = $this->areaProvider->getArea($player->getWorld(), $player->getLocation()->getX(), $player->getLocation()->getZ(), strtolower($player->getName()));
